 ...
 * @author Marek Cuchý
 */
public class SimplifiedEdgeBuilder {

	public static final Comparator<SimplifiedEdgeBuilder> LENGTH_COMPARATOR = ((o1, o2) -> Double.compare(o1.length,
			o2.length));
	...
	private SimplifiedEdgeBuilder(int f, int n, int t, Type type) {
		this.type = type;
		this.tmpFromId = f;
		this.tmpToId = t;
		removedNodes.add(n);
	 ...
	 * @param postfix
	 */
	public final void append(SimplifiedEdgeBuilder postfix) {
	 ...
	 * @param postfix
	 */
	public final void appendReversed(SimplifiedEdgeBuilder postfix) {
	 ...
	 * @param prefix
	 */
	public void prependReversed(SimplifiedEdgeBuilder prefix) {
	...
	public static SimplifiedEdgeBuilder createOneway(int f, int n, int t) {
		return new SimplifiedEdgeBuilder(f, n, t, Type.ONEWAY);
	}

	public static SimplifiedEdgeBuilder createTwoWay(int f, int n, int t) {
		return new SimplifiedEdgeBuilder(f, n, t, Type.TWO_WAY);
	}
	 ...
	 * @param numberOfParts
	 * @param graph
	 * 		Graph from which are retrieved locations of nodes and properties of the edges.
	 * @param removedEdges
	 *
	 * @return
	 */
	public List<RoadEdgeBuilder> build(int numberOfParts, TmpGraphBuilder<RoadNode, RoadEdge> graph,
		...
		switch (numberOfParts) {
			...
			case 2:
				List<RoadEdgeBuilder> result2 = new ArrayList<>(2);
				...
				return result2;
			case 3:
				List<RoadEdgeBuilder> result3 = new ArrayList<>(3);
	...
	private List<RoadEdgeBuilder> build(List<Integer> nodes, TmpGraphBuilder<RoadNode, RoadEdge> graph,
										Map<EdgeId, RoadEdgeBuilder> removedEdges) {
		int length = (int) Math.round(calculateLength(nodes, graph));
		List<RoadEdgeBuilder> result = new ArrayList<>(2);
		switch (type) {
			case TWO_WAY:
	...
	private RoadEdgeBuilder createEdge(List<Integer> nodes, int length, Map<EdgeId, RoadEdgeBuilder> removedEdges) {
		int from = nodes.get(0);
		int m = nodes.get(1);
		int to = nodes.get(nodes.size() - 1);
		RoadEdgeBuilder referenceEdge = removedEdges.get(new EdgeId(from, m));
	...
	public double calculateAndSetLength(TmpGraphBuilder<RoadNode, RoadEdge> graph) {
		List<Integer> nodes = new ArrayList<>(numberOfNodes());
		nodes.add(getTmpFromId());
		nodes.addAll(removedNodes);
		nodes.add(getTmpToId());
	...
	public boolean equalType(SimplifiedEdgeBuilder b) {
		return type == b.type;
	}
